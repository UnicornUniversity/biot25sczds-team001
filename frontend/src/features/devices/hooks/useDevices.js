...
export default function useDevices(currentBuildingId) {
  ...
  useEffect(() => {
    run(async () => {
      ...
      // 2) Load controllers attached to this building's gateways
  ...
  }, [ownerId, currentBuildingId, run, success]);
  ...
  const updateController = useCallback((id, data) =>
    ...
    .then(dev => {
      setControllers(d => d.map(x => x._id === dev._id ? dev : x));
    })
    .catch(err => error(msgs.devices.updateError.replace('{error}', err.message))),
  [run, error]);

// uvnitř useDevices, nahraď detachController touto implementací
const detachController = useCallback((id) =>
  run(async () => {
    // 1) PUT /devices/:id se soft‐detach
    ...
    // 2) odstraníme z lokálního stavu
    setControllers(prev => prev.filter(c => c._id !== id));

    return dev;
  }, msgs.devices.delete)
  .catch(err => {
    error(msgs.devices.deleteError.replace('{error}', err.message));
    throw err;
  })
, [run, error]);
